...
python "$_script" [--spec PATH_OR_URL] [--refresh] [--raw] [--depth N] <subcommand> [args]
```
...
If `openapi.json` is not found, suggest running with `--spec` before the subcommand — either a local path or a URL, e.g. `python "$_script" --spec path/to/spec.json summary` or `--spec https://api.example.com/openapi.json summary` (where `$_script` is resolved as above).
